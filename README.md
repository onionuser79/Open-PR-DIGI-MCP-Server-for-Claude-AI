...
- **On-demand route discovery (`remote_run`):** reach **any** callsign — even one
  ...
  and NetROM-destination (`N`) tables, ranks the candidate routes (FlexNet routes
  first, by cost; then NetROM, by hops then quality), connects over AX.25 (`C`) to
  the best (falling back to the next on failure), recognises the far node's family
  (from its MOTD, else `V`), and runs your command in that family's own command set.
